document.addEventListener("DOMContentLoaded", () => {
  ...
  const graph_infomationSpace = document.getElementById(
    "js-graph_infomationSpace"
  );
  ...
  const graph_managementSpace = document.getElementById(
    "js-graph_managementSpace"
  );
    ...
    fetch(`/graph?age=${age}`).then((res) => {
      ...
      if (res.status === 400) {
        ...
        res.json().then((data) => {
          console.log(data);
          //データの初期化
          graph_databaseSpace.style.width = '0%';
          graph_database.textContent = '0%';
          graph_systemSpace.style.width = '0%';
          graph_system.textContent = '0%';
          graph_infomationSpace.style.width = '0%';
          graph_infomation.textContent = '0%';
          graph_networkSpace.style.width = '0%';
          graph_network.textContent = '0%';
          graph_managementSpace.style.width = '0%';
          graph_management.textContent = '0%';
          graph_securitySpace.style.width = '0%';
          graph_security.textContent = '0%';
          data.forEach((dataEach) => {
